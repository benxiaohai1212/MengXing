// Ver: 4.1
var nOP = 0, nOP5 = 0, nIE = 0, nIE4 = 0, nIE5 = 0, nNN = 0, nNN4 = 0, nNN6 = 0, nMAC = 0, nIEM = 0, nIEW = 0, nDM = 0, nVER = 0.0, st_delb = 0, st_addb = 0, st_reg = 1;
stnav();
var st_ttb = nIE || nOP && (nVER >= 6 && nVER < 7);
var stT2P = [ "static", "absolute", "absolute" ], stHAL = [ "left", "center",
		"right" ], stVAL = [ "top", "middle", "bottom" ], stREP = [
		"no-repeat", "repeat-x", "repeat-y", "repeat" ], stBDS = [ "none",
...
p.is[st_ci]=
{
			itex : a[0] ? a[1] : a[1].replace(new RegExp(" ", "g"),
					nMAC ? "&#160;" : "&nbsp;"),
			...
			ihal : stHAL[a[21]],
			ival : stVAL[a[22]],
			ibgc : nOP5 && nVER < 7.0 && a[24] && a[26] ? [ "transparent",
					"transparent" ] : [
					nOP5 && nVER < 7.0 || !a[24] ? a[23] : "transparent",
					nOP5 && nVER < 7.0 || !a[26] ? a[25] : "transparent" ],
			ibgi : [ stbuf(stgsrc(a[27], m, 0)), stbuf(stgsrc(a[28], m, 0)) ],
			ibgr : [ stREP[a[29]], stREP[a[30]] ],
			ibds : stBDS[a[31]],
			ipbw : a[32],
			ibdc : (!nDM || nNN4) ? [ a[33].split(st_rsp)[0],
					a[34].split(st_rsp)[0] ] : [ a[33], a[34] ],
			itxc : [ a[35], a[36] ],
			itxf : [ a[37], a[38] ],
	...
	it.tmid = null;
	if (it.ityp != 6)
		it.ibgd = [ stgbg(it.ibgc[0], it.ibgi[0], it.ibgr[0]),
				stgbg(it.ibgc[1], it.ibgi[1], it.ibgr[1]) ];
	eval(l + "=it;");
}
...
function stm_em() {
	...
	for (var ppi = 0; ppi < l; ppi++) {
		...
		if (p.isst && m.maln != "left")
			ph = "<TABLE STYLE='border:none;padding:0px;' CELLPADDING=0 CELLSPACING=0 ALIGN="
					+ m.maln + "><TD class=st_tdcss>" + ph;
...
function stpbtx(p) {
	...
	if (nNN4 || !nDM) {
		...
		var ds = "position:"
		http: // www.zstmachinery.com/Inc/+stT2P[p.ppi?1:stgme(p).mtyp]+";";
		ds += "z-index:" + p.zind + ";";
		ds += "visibility:hidden;";
		s += st_ttb ? "<TABLE class=st_tbcss CELLPADDING=0 CELLSPACING=0"
				: "<DIV class=st_divcss";
		s += stppev(p);
		s += " ID=" + p.ids;
		s += " STYLE='";
...
function stittx(i) {
	...
	if (nNN4 || !nDM) {
		...
		for (var l = 0; l < (nNN4 ? 2 : 1); l++) {
			...
			s += ">";
			s += "<TABLE ALIGN=LEFT WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING="
					+ (i.ityp == 6 ? 0 : p.ppad);
			s += " BACKGROUND='" + (i.ityp != 6 ? i.ibgi[l] : "") + "'";
			s += " BGCOLOR="
					+ (i.ityp != 6 && i.ibgi[l] || i.ibgc[l] == "transparent" ? "''"
							: i.ibgc[l])
			s += ">";
			if (i.ityp == 6) {
				s += "<TD NOWRAP VALIGN=TOP" + " HEIGHT="
						+ (p.pver ? i.ssiz : "100%") + " WIDTH="
						+ (p.pver ? "100%" : i.ssiz)
						+ " STYLE='font-size:0pt;'" + ">";
				...
				s += "<TD WIDTH=100% NOWRAP ALIGN=" + i.ihal + " VALIGN="
						+ i.ival + ">";
				s += "<A " + stgurl(i) + " CLASS='" + (i.ids + "TX" + l) + "'>";
				if (i.ityp == 2)
					s += stgimg(i.iimg[l], i.ids + "IMG", i.iimw, i.iimh,
							i.iimb);
				else {
					s += "<IMG SRC=\"" + stgme(i).mbnk
							+ "\" WIDTH=1 HEIGHT=1 BORDER=0 ALIGN=ABSMIDDLE>";
					s += i.itex;
				}
				s += "</A>";
				s += "</TD>";
			...
			s += "</TABLE>";
			if (i.ipbw && i.ityp != 6)
				s += "<BR CLEAR=ALL><SPACER HEIGHT=1 WIDTH=" + i.ipbw
						+ "></SPACER><SPACER WIDTH=1 HEIGHT=" + i.ipbw
						+ "></SPACER>";
			s += "</LAYER>";
		}
		if (i.ityp != 6)
		...
		s += ">";
		if (i.ityp == 6) {
			s += "<TD class=st_tdcss NOWRAP VALIGN=TOP" + " ID=" + i.ids
					+ "MTD" + " HEIGHT=" + (p.pver ? i.ssiz : "100%")
			...
			s += " ALIGN=" + i.ihal;
			s += " VALIGN=" + i.ival + ">";
			s += "<FONT class=st_ftcss ID=" + i.ids + "TX STYLE=\""
					+ sttcss(i, 0) + "\">";
			if (i.ityp == 2)
	...
	if (nIEW && (nVER >= 5.0 || !p.isst)) {
		...
		if (nVER >= 5.5) {
			...
			if (p.pesi < 24 && p.pesi >= 0)
				s += "revealTrans(Transition=" + p.pesi + ",Duration="
						+ ((110 - p.pesp) / 100) + ") ";
		}
		s += dx + "Alpha(opacity=" + p.popc + ") ";
		if (p.psds) {
			if (p.pstp == 1)
				s += dx + "dropshadow(color=" + p.pscl + ",offx=" + p.psds
						+ ",offy=" + p.psds + ",positive=1) ";
			else
				s += dx + "Shadow(color=" + p.pscl + ",direction=135,strength="
						+ p.psds + ") ";
		}
		if (nVER >= 5.5) {
			if (p.pehd != "Normal")
			...
			if (p.pehi < 24 && p.pehi >= 0)
				s += "revealTrans(Transition=" + p.pehi + ",Duration="
						+ ((110 - p.pesp) / 100) + ") ";
		}
		s += ";";
	}
...
function stitov(e, o, i) {
	if (nIEW) {
		...
		if (!stgpar(i).issh
				|| (!nNN && (e.fromElement && e.fromElement.id && e.fromElement.id
						.indexOf(i.ids) >= 0)))
			return;
	}
	if (nNN4)
...
function stitou(e, o, i) {
	if (nIEW) {
		...
		if (!stgpar(i).issh
				|| (!nNN && (e.toElement && e.toElement.id && e.toElement.id
						.indexOf(i.ids) >= 0)))
			return;
	}
	if (nNN4)
...
function stshst(i, n) {
	if (nNN4) {
		...
		if (nIEM) {
			if (i.ibgd[0] != i.ibgd[1])
				os.background = i.ibgd[n];
		} else {
			...
			if (i.ibgi[0] != i.ibgi[1])
				os.backgroundImage = "url("
						+ (i.ibgi[n] ? i.ibgi[n] : stgme(i).mbnk) + ")";
			if (i.ibgr[0] != i.ibgr[1])
				os.backgroundRepeat = i.ibgr[n];
		}
		if (i.ibdc[0] != i.ibdc[1])
			os.borderColor = i.ibdc[n];
		var t;
		...
		if (!i.txstyle)
			i.txstyle = stgobj(i.ids + "TX", 'FONT').style;
		t = i.txstyle;
		...
		if (nOP)
			stgobj(i.ids + 'MTD', 'td').style.color = i.itxc[n];
		else
			t.color = i.itxc[n];
	}
}
	...
	if (nNN4) {
		...
		if (!p.fixed) {
			...
			for (var l = p.is.length - 1; l >= 0; l--) {
				...
				if (i.ityp != 6) {
					var ls = stgstlay(i);
					ls[2].resizeTo(ls[0].parentLayer.clip.width,
							ls[0].parentLayer.clip.height);
					if (stgcur(i) == "hand") {
						with (ls[2].document) {
							open();
							write("<A " + stgurl(i) + "\"><IMG BORDER=0 SRC='"
									+ stgme(i).mbnk + "' WIDTH="
									+ ls[2].clip.width + " HEIGHT="
									+ ls[2].clip.height + "></A>");
							close();
						}
					}
			...
			if (p.ipbw)
				setTimeout("var pp=st_ms[" + p.mei + "].ps[" + p.ppi
						+ "];stglay(pp).bgColor=pp.pbdc;", 1);
			p.fixed = 1;
		}
	} else {
...
function stnav() {
	...
	if (nOP) {
		...
		if (nIE) {
			...
			nNN4 = navigator.appName.toLowerCase() == "netscape"
					&& v.substring(0, 1) == "4" ? 1 : 0;
			if (!nNN4) {
				nNN6 = (document.getElementsByTagName("*") && a
						.indexOf("Gecko") != -1);
				if (nNN6) {
...
function stpre(m) {
	...
	if (nIE || nNN6)
		window.onscroll = new Function(
				"if(st_scr)clearTimeout(st_scr);st_scr=setTimeout('stscr();',nIEM ?500:0);");
	else if (!st_rl) {
		st_cw = stgcw();
		st_ch = stgch();
		st_cl = stgcl();
...
function stsdstr(p, iss) {
	return "var pp=st_ms[" + p.mei + "].ps[" + p.ppi + "];pp.tmid=null;"
			+ (iss ? p.efsh + "sh(" : p.efhd + "hd(") + "pp);pp.exed=1;"
}
function stwover(e, o) {
	...
	return ((l < o.rc[2] + o.rc[0]) && (l + w > o.rc[0])
			&& (t < o.rc[3] + o.rc[1]) && (t + h > o.rc[1]));
}
function stevfn(pr, isi) {
	var s = isi ? 'st_ri' : 'st_rp';
	s += '.exec(this.parentLayer.id);mei=RegExp.$1;ppi=parseInt(RegExp.$2);';
	if (isi)
		s += 'iti=parseInt(RegExp.$3);return ' + pr
				+ '(e,this,st_ms[mei].ps[ppi].is[iti]);';
	else
		s += 'return ' + pr + '(e,this,st_ms[mei].ps[ppi]);';
...
function stitev(i) {
	if (i.ityp == 6)
		return '';
	var s = " onMouseOver='stitov(event,this,st_ms[" + i.mei + "].ps[" + i.ppi
			+ "].is[" + i.iti + "]);'";
	s += " onMouseOut='stitou(event,this,st_ms[" + i.mei + "].ps[" + i.ppi
			+ "].is[" + i.iti + "]);'";
	s += " onClick='stitck(event,this,st_ms[" + i.mei + "].ps[" + i.ppi
			+ "].is[" + i.iti + "]);'";
	return s;
}
function stquo(n) {
...
function stgurl(i) {
	return " HREF="
			+ stquo(i.iurl == "" ? "#" : i.iurl.replace(new RegExp("\"", "g"),
					"&quot;"))
			+ " TARGET="
			+ stquo(i.iurl == ""
					|| i.iurl.toLowerCase().indexOf('javascript:') == 0 ? "_self"
					: i.itgt);
}
function stgdec(v) {
	if (v) {
...
function stgcur(i) {
	if (nNN6)
		return "default";
	return i.ityp != 6
			&& ((i.ppi == 0 && stgme(i).mcks && stgsub(i)) || i.iurl) ? "hand"
			: "default";
}
function stgiws(o) {
	if (stgpar(o).pver)
...
function stgcw() {
	return parseInt(nNN || nOP ? window.innerWidth
			: (nIEW && document.compatMode == "CSS1Compat" ? document.documentElement.clientWidth
					: document.body.clientWidth));
}
function stgch() {
	return parseInt(nNN || nOP ? window.innerHeight
			: (nIEW && document.compatMode == "CSS1Compat" ? document.documentElement.clientHeight
...
function stglay(ip) {
	if (!ip.layer) {
		if (typeof (ip.iti) == 'undefined')
			...
			ip.layer = nNN4 ? stglay(stgpar(ip)).document.layers[0].document.layers[ip.ids]
					: stgobj(ip.ids, nIEW ? 'table' : null);
	}
	return ip.layer;
}
function stgstlay(i) {
...
function stgrc(ip) {
	if (nNN4) {
		...
		var l = 0, t = 0;
		var ly = stglay(ip);
		var w = typeof (ip.rc) == "undefined" ? parseInt(nOP && nVER < 7 ? ly.style.pixelWidth
				: ly.offsetWidth)
				: ip.rc[2];
		var h = typeof (ip.rc) == "undefined" ? parseInt(nOP && nVER < 7 ? ly.style.pixelHeight
				: ly.offsetHeight)
				: ip.rc[3];
		while (ly) {
...
function stgsrc(s, m, f) {
	...
	if (sr.indexOf('http://') == 0
			|| (sr.indexOf(':') == 1 && sr.charCodeAt(0) > 96 && sr
					.charCodeAt(0) < 123) || sr.indexOf('ftp://') == 0
			|| sr.indexOf('/') == 0 || sr.indexOf('gopher') == 0)
		return s;
	else
		return m.mweb + s;
}
function showFloatMenuAt(n, x, y) {
	if (nDM) {
		var m = stmenu(n);
		if (m && typeof (m.ready) != "undefined" && m.mtyp == 2 && m.ps.length
				&& !m.ps[0].issh) {
			ststxy(m, [ x, y ]);
			stshow(m.ps[0]);
		}
